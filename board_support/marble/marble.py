...
from litex.boards.platforms import marble
...
class _CRG(Module):
    def __init__(self, platform, sys_clk_freq):
        ...
        pll.register_clkin(platform.request("clk20_vcxo"), 20e6)
    ...
    def __init__(self, phy="rgmii", **kwargs):
        ...
        if phy == "rgmii":
            ...
            self.platform.add_period_constraint(
                self.ethphy.crg.cd_eth_rx.clk, 1e9/125e6)
            self.platform.add_period_constraint(
                self.ethphy.crg.cd_eth_tx.clk, 1e9/125e6)
        ...
        if phy == "1000basex":
            ...
            self.submodules.ethphy = A7_1000BASEX(
                qpll.channels[0], self.platform.request("sfp", 0), self.clk_freq)
            self.platform.add_period_constraint(
                self.ethphy.txoutclk, 1e9/62.5e6)
            self.platform.add_period_constraint(
                self.ethphy.rxoutclk, 1e9/62.5e6)
            self.platform.add_false_path_constraints(
                self.crg.cd_sys.clk,
