...
The build system now supports multiple configurations, depending on the variable `CONFIG`.
Supported targets include `marblemini`, `marble`, and `marble_fiber`.
The `marble_fiber` enables GigE over fiber, while the rest over copper.
If no configuration is specified, it defaults to `marblemini`.
