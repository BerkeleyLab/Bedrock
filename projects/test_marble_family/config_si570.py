...
def config_si570(dev, addr):
    if args.new_freq:
        ...
        compute_si570(dev, addr)
    else:  # read only current settings
        compute_si570(dev, addr)
...
if __name__ == "__main__":
    ...
    dev = lbus_access.lbus_access(args.ip, port=args.port, timeout=3.0, allow_burst=False)

    config_si570(dev, addr)

# usage:
# To read current output frequency:
# python3 config_si570.py --ip 192.168.19.31 --port 803 --debug
# To change output frequency:
# python3 config_si570.py --ip 192.168.19.31 --port 803 --debug -f 185
