...
from configuration.jtag import SPI_Base_Exception, xilinx_bitfile_parser
import spi
import time
...
class interface():
    ...
    def get_images(self):
        ...
        data = self.__target.read_data(KINTEX_IMAGE_TABLE_ADDRESS,
                                       spi.SECTOR_SIZE)
    ...
    def save_image_table(self, images):
        ...
        for i in range(0, len(table) / 256):
            self.__target.page_program(table[i * 256:(i + 1) * 256],
                                       i * 256 + KINTEX_IMAGE_TABLE_ADDRESS)
        ...
        for i in range(0, len(pd)):
            if (table[i] != pd[i]):
                print()
                raise SPI_Base_Exception('Image table update byte',
                                         str(i), 'failed')
    ...
    def add_image(self, name):
        ...
        print('Sectors needed: {}'.format(length))
        ...
        if location == 0:
            raise spi.SPI_Base_Exception(
                'Insufficient contiguous sectors available on device')

        print('Writing image starting at sector {}'.format(location))
        ...
        build_date = int(
            time.mktime(
                datetime.strptime(parser.build_date() + ' ' +
                                  parser.build_time(), '%Y/%m/%d %H:%M:%S')
                .timetuple()))

        print('Storage timestamp: {}'.format(storage_date))
        print('Build timestamp: {}'.format(build_date))
