...
class bitfile():
    def __init__(self, file):
        with open(file, "rb") as f:
            ...
            if bytes == b"":
                raise Xilinx_Bitfile_Parser_Exception(
                    'Incorrect design file name length')
            ...
            name_length = (int(bytes[0]) << 24) + (int(bytes[1]) << 16) + (
            ...
            if name_length != len(result):
                raise Xilinx_Bitfile_Parser_Exception(
                    'Bitstream length doesn\'t match rest of file')
