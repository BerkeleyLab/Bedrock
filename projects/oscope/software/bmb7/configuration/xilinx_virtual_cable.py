...
import socket
import sys
from configuration.jtag import jtag
...
def jtag_step(s, tms):
    ...
    states = {
        jtag.states.TEST_LOGIC_RESET:
        [jtag.states.RUN_TEST_IDLE, jtag.states.TEST_LOGIC_RESET],
        jtag.states.RUN_TEST_IDLE:
        [jtag.states.RUN_TEST_IDLE, jtag.states.SELECT_DR_SCAN],
        jtag.states.SELECT_DR_SCAN:
        [jtag.states.CAPTURE_DR, jtag.states.SELECT_IR_SCAN],
        ...
        jtag.states.UPDATE_DR:
        [jtag.states.RUN_TEST_IDLE, jtag.states.SELECT_DR_SCAN],
        jtag.states.SELECT_IR_SCAN:
        [jtag.states.CAPTURE_IR, jtag.states.TEST_LOGIC_RESET],
    ...
    while True:
        ...
        if data == 'ge':
            ...
            if data[0:5] != 'ttck:':
                return False
            v = bytearray(data)
            print('Received settck:', int(v[8]) * 16777216 + int(
                v[7]) * 65536 + int(v[6]) * 256 + int(v[5]))
            reply = bytearray(4)
            for i in range(0, 4):
                reply[i] = v[5 + i]
            c.send(str(reply))
        ...
        # TODO Endian check?
        num_bits = bytearray([data[4]])[0] + bytearray(
            [data[5]])[0] * 256 + bytearray([data[6]])[0] * 65536 + bytearray(
                [data[7]])[0] * 16777216
        ...
        seen_tlr = (seen_tlr or
                    (jtag_state == jtag.states.TEST_LOGIC_RESET)) and (
                        jtag_state != jtag.states.CAPTURE_DR) and (
                            jtag_state != jtag.states.CAPTURE_IR)

        # Apparently Impact can go through capture after reading IR/DR which triggers IR
        # to read out an IR value, ignore these...
        if (((jtag_state == jtag.states.EXIT_1_IR) and (num_bits == 5) and
             (data[0] == 0x17)) or ((jtag_state == jtag.states.EXIT_1_DR) and
                                    (num_bits == 4) and (data[0] == 0x0b))):
