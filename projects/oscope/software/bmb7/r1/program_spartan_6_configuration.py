...
from configuration.spi import spi

CONFIG_ADDRESS = 24 * spi.SECTOR_SIZE

parser = argparse.ArgumentParser(
    description='Program BMB7 Spartan-6 boot configuration',
...
parser.add_argument(
    '-i', '--ip', help='Unicast IP address to be written into flash')
parser.add_argument(
    '-m', '--mac', help='Unicast MAC address to be written into flash')
...
print('PROM ID (0x20BA, Capacity=0x19, EDID+CFD length=0x10, EDID (2 bytes), CFD (14 bytes)', end=' ')

print('VCR (should be 0xfb by default):', hex(
    prom.read_register(spi.RDVCR, 1)[0]), end=' ')
print('VECR (should be 0xdf):', hex(prom.read_register(spi.RDVECR, 1)[0]))

if prom.prom_size() != 25:
    print('PROM size incorrect, read', spi.interface.prom_size())
    exit()
...
# Set the BMB7 unicast MAC either a new value, the current one if PROM checksum is valid,
# or AA:BB:CC:DD:EE:FF as default.
...
# Set the Kintex-7 boot firmware hash to either a new value, the current one if PROM checksum is
# valid, or NULL as default.
if (args.hash is not None):
    ...
    print('PROM configuration checksum is invalid - setting Kintex-7 boot firmware SHA256'
          'hash to 00000000000000000000000000000000')
