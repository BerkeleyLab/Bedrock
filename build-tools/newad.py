...
class Newad:
    ...
    def __init__(self):
        ...
        self.hierarchy = ["xxxx", "station", "cav4_elec", ["mode_", 3]]  # TODO

    def generate_mirror(self, dw, mirror_n):
        """
    ...
    def add_to_global_map(self, name, base_addr, sign, aw, dw, description):
        if name in self.flat_addr_map:
            x = self.flat_addr_map[name]
            assert x["base_addr"] == base_addr
        self.flat_addr_map[name] = {
            "base_addr": base_addr,
            "sign": sign,
    ...
    def generate_addresses(
        self, fd, names, base, low_res=False, gen_mirror=False, plot_map=False
    ):
        """
        ...
        mirror_base = -1
        register_names = sorted(names, key=lambda x: self.ch.get(x)[0], reverse=True)
        if gen_mirror:
            mirror_size = sum(
                [
                    1 << self.ch[k][0]
                    for k in register_names
                    if (1 << self.ch[k][0]) <= MIN_MIRROR_ARRAY_SIZE
                ]
            )
        for k in register_names:
            bitwidth = self.ch[k][0]
            register_array_size = 1 << bitwidth
            if (
                gen_mirror
                and mirror_base == -1
                ...
                if fd:
                    ...
                    fd.write(s)
            sign = self.ch[k][2]
            datawidth = self.ch[k][3]
            description = self.ch[k][-1]
            ...
            if bitwidth > 0 and not low_res:
                ...
                self.add_to_global_map(k, next_addr, sign, bitwidth, datawidth, description)
            if fd:
                s = (
    ...
    def address_allocation(
            self, fd, hierarchy, names, address, low_res=False, gen_mirror=False, plot_map=False
    ):
        """
        ...
        hierarchy: Index into self.hierarchy (current hierarchy level)
        names: All signal names that belong in the current hierarchy
        address:
        for current index in self.hierarchy denoted with variable 'hierarchy'
        ...
        """
        if hierarchy == len(self.hierarchy):
            return self.generate_addresses(fd, names, address, low_res, gen_mirror, plot_map)
        ...
        if type(h) is list:
            ...
            for hn in range(h[1]):
                hh = prefix + str(hn)
                address = self.address_allocation(
                    fd,
                    hierarchy + 1,
        ...
        return self.generate_addresses(fd, out_mod, address, low_res, gen_mirror, plot_map)


    def parse_and_print_decode_header(self, fi, modname, fo, dir_list, lb_width, gen_mirror, use_yosys):
        clk_prefix = "lb"
        obuf = StringIO()
        if use_yosys:
            vfile_parser = Parser()
        ...
        self.ch = vfile_parser.gch
        self.flat_addr_map = vfile_parser.g_flat_addr_map
        self.file_not_found = vfile_parser.file_not_found
        obuf.write("// machine-generated by newad.py\n")
        obuf.write("`ifdef LB_DECODE_%s\n" % modname)
        obuf.write('`include "addr_map_%s.vh"\n' % modname)
        # TODO: Merging clock domains: This eventually doesn't need to be here
        ...
        if gen_mirror:
            obuf.write(self.generate_mirror(32, 0))
        obuf.write("\n")
        obuf.write("`else\n")
        ...
        obuf.write("`define AUTOMATIC_decode\n")
        obuf.write("`endif\n")
        # Below applies for modules with genvar constructions
    ...
    def write_address_header(
            self, output_file, low_res, lb_width, gen_mirror, base_addr, plot_map
    ):
        addr_bufs = StringIO()
        addr_bufs.write("`define LB_HI %d\n" % lb_width)
        self.address_allocation(
            addr_bufs, 0, sorted(self.ch.keys()), base_addr, low_res, gen_mirror, plot_map
        )
    ...
    def write_regmap_file(self, output_file, low_res, gen_mirror, base_addr, plot_map):
        self.address_allocation(
            fd=0,
            hierarchy=0,
            names=sorted(self.ch.keys()),
            address=base_addr,
            low_res=low_res,
            gen_mirror=gen_mirror,
            plot_map=plot_map,
        )
        addr_map = {x: self.flat_addr_map[x] for x in self.flat_addr_map}
        with open(output_file, "w") as fd:
...
def main(argv):
    ...
    if newad.file_not_found > 0:
        print(newad.file_not_found, "files not found")
        exit(1)
